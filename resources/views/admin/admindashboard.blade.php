...
<body class="bg-gradient-to-br from-gray-50 to-gray-100">
                ...
                <ul>
                    ...
                    <li>
                        <a href="/admin/dashboard"
                            class="flex items-center px-6 py-2 bg-cyan-50 text-cyan-700 group border-r-4 border-cyan-600">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-cyan-600" fill="none"
    ...
    <div class="ml-20 transition-all duration-300">
        ...
        <header
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <img src="https://cdn-icons-gif.flaticon.com/10606/10606611.gif" class="w-8 h-8 rounded-lg"
                        alt="Logo">
                    <h1
        ...
        <div id="dashboard-content" class="container mx-auto px-4 py-8 hidden">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <!-- Total Shops Card -->
                <div
                    class="relative overflow-hidden bg-white rounded-2xl shadow-lg border border-gray-100 p-6 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                    ...
                    <div class="relative">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span
                                class="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-semibold rounded-full">Live</span>
                        </div>
                ...
                <div
                    class="relative overflow-hidden bg-white rounded-2xl shadow-lg border border-gray-100 p-6 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                    <div class="absolute top-0 right-0 w-32 h-32 bg-emerald-50 rounded-full -mr-16 -mt-16 opacity-50">
                    </div>
                    <div class="relative">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span
                                class="px-3 py-1 bg-emerald-50 text-emerald-700 text-xs font-semibold rounded-full">Active</span>
                        </div>
                ...
                <div
                    ...
                    <div class="relative">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span
                                class="px-3 py-1 bg-purple-50 text-purple-700 text-xs font-semibold rounded-full">Catalog</span>
                        </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    <div class="bg-gradient-to-r from-purple-50 to-purple-100 px-8 py-6 border-b border-gray-200">
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 bg-purple-600 rounded-lg flex items-center justify-center shadow-md">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-white" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 mt-0.5">Add and manage lenses available across all
                                        shops</p>
                                </div>
                            </div>
                        </div>
            ...
            <form id="lensForm" enctype="multipart/form-data">
                ...
                <div class="flex gap-4 mt-6">
                    ...
                    <button type="button" onclick="closeLensModal()"
                        class="flex-1 bg-gray-300 text-gray-700 py-3 rounded-lg font-semibold hover:bg-gray-400 transition-all">
                        Cancel
                    </button>
                </div>
            </form>
    ...
    lensForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        const token = localStorage.getItem('auth_token');
        const formData = new FormData(lensForm);
        const lensId = document.getElementById('lensId').value;

        // Admin API routes ke mutabiq endpoints
        let endpoint = '/api/admin/lenses'; 
        
        // Agar Edit kar rahe hain to method spoofing use karein
        if (lensId) {
            endpoint = `/api/admin/lenses/${lensId}`;
            formData.append('_method', 'PUT'); 
        }

        try {
            const response = await fetch(endpoint, {
                method: 'POST', // Hamesha POST use karein FormData ke saath
                headers: {
                    'Authorization': 'Bearer ' + token,
                    'Accept': 'application/json' // Isse HTML error ke bajaye JSON milega
                },
                body: formData
            });
            ...
            if (response.ok && result.success) {
                alert('Lens saved successfully!');
                closeLensModal();
                loadLenses(); // List refresh karein
            } else {
                // Agar Validation fail hui (e.g. type galat hai)
